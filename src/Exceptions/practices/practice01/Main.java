package Exceptions.practices.practice01;
...
import Exceptions.practices.practice01.entities.Reservation;
...
public class Main {
    public static void main(String[] args) throws ParseException {
        ...
        if (getNewDateCheckIn.before(dateNow) || getNewDateCheckOut.before(dateNow)) {
            System.out.println("Error in reservation: Reservation dates for update must be future dates.");
            return;
        }
        if (!getNewDateCheckOut.after(getNewDateCheckIn)) {
            System.out.println("Error in reservation: Check-out date must be after check-in date");
            return;
        }
